...
| 调用 brainstorming 时根据目录结构或 git 历史推断主题内容，补充假设性"背景"字段 | 严格按模板注入上下文（主题名 + 结构约束），主题学习范围由 brainstorming 向用户提问确定，不得自行推断 |
| 在子目录中运行 `git` 命令探索项目上下文（`git log`、`git ls-tree` 等），未意识到 `.git` 在父目录，实际读取的是父目录 repo 的内容 | 运行 git 命令前先确认当前目录是否有 `.git`（`ls -la | grep .git`）；若无，只用 `ls` 查看当前目录实际文件，不用 git 命令探索上下文 |
